...
const (
	...
	defaultMemory        = 1024
	defaultNFSSharesRoot = "/hyperkit-nfsshares"
)
...
func (d *Driver) GetCreateFlags() []mcnflag.Flag {
	return []mcnflag.Flag{
		...
		mcnflag.StringFlag{
			EnvVar: "HYPERKIT_NFS_SHARE_ROOT",
			Name:   "hyperkit-nfs-share-root",
...
func (d *Driver) SetConfigFromFlags(flags drivers.DriverOptions) error {
	...
	d.NFSSharesRoot = flags.String("hyperkit-nfs-share-root")
...
func (d *Driver) createHost() (*hyperkit.HyperKit, error) {
	...
	h.Kernel = d.ResolveStorePath("kernel")
...
func (d *Driver) setupNFSShare() error {
	...
	for _, share := range d.NFSShares {
		...
		mountCommands += fmt.Sprintf("sudo mount -t nfs -o noacl,async,nolock,nfsvers=3 %s:%s %s/%s\\n", hostIP, share, root, share)
	}
